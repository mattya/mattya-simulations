...
int getRayTrace(Ray r, Tri[] trilist, Vec ans){
  float tmpt = INF;
  int tmpk = -1;
  for(int k=0; k<N; k++){
...
//  tris[19] = new Tri(new Vec(INF, INF, INF), new Vec(INF, INF, INF), new Vec(INF, INF, INF));
  ...
  Vec lightVec = new Vec(1.0, 1.2, -1);
  lightVec.normalize();
  ...
  for(int i=0; i<pi.height; i++){
    for(int j=0; j<pi.width; j++){
      ...
      ry.normalize();
      c = color(0);
      Vec tmpv = new Vec(1, 0, 0);
      int tmpk = getRayTrace(ry, tris, tmpv);
//          println(tmpk + " " + tmpv.x + " " + tmpv.y + " " + tmpv.z);
      if(tmpk>=0){
        float a = tris[tmpk].getH().angle(lightVec);
        if(-cos(a)>0.1){
          c = tris[tmpk].c;
          Vec kouten = (ry.dir.mult(tmpv.x)).add(ry.ori);
//          println(kouten.x + " " + kouten.y + " " + kouten.z);
          Vec dirLight = lightVec.mult(-1.0);
//          println(dirLight.x + " " + dirLight.y + " " + dirLight.z);
          Vec tmpv2 = new Vec(0, 0, 0);
          int tmpk2 = getRayTrace(new Ray(kouten.add(tris[tmpk].getH().mult(0.01)), dirLight), tris, tmpv2);
//          tmpk2 = -1;
//          int tmpk2 = getRayTrace(new Ray(kouten, dirLight), tris, tmpv2);
//          println(i+" " + j + " " + tmpk + " " + tmpk2 + " : " + tmpv2.x + " " + tmpv2.y + " " + tmpv2.z);
          if(tmpk2==-1){
