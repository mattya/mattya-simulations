...
color getColor(Ray r, Obj[] objlist, Vec lightVec, float rr){
  ...
  if(f==-1){
    ...
    // touka no shori
    if(rr*target.tra > 0.1){
      kouten = kouten.sub(nvec.mult(0.01));
      float n = 1.0/target.n;
      if(cos(a)>0){
        n = target.n;
        nvec = nvec.mult(-1);
      }
      kouten = kouten.sub(nvec.mult(0.01));
      float nd = -nvec.dot(r.dir);
      float abn = nvec.abs();
      float abnd = abn*r.dir.abs();
      Vec tmp = nvec.mult(-nd/abn/abn);
      float rat = nd/abnd;
      color c2;
      if(n*sqrt(1-rat*rat) > 1.0){
        kouten = kouten.add(nvec.mult(0.02));
        c2 = getColor(new Ray(kouten, (r.dir.add(nvec.mult(2.0*nd/abnd/abnd)))), objlist, lightVec,  rr*target.tra);
      }else{
        Vec d2 = tmp.add(r.dir.sub(tmp).mult(n*rat/sqrt(1+n*n*(rat*rat-1))));
//        println(r.dir.x + " " + r.dir.y + " " + r.dir.z + " : " + d2.x + " " + d2.y + " " + d2.z);
        c2 = getColor(new Ray(kouten, d2), objlist, lightVec, rr*target.tra);
      }
      c = color((1.0-target.tra)*red(c) + target.tra*red(c2),
                (1.0-target.tra)*green(c) + target.tra*green(c2),
                (1.0-target.tra)*blue(c) + target.tra*blue(c2));
...
void setup(){
  ...
  tex = loadImage("../tex4.jpg");
  tex.loadPixels();
}

void draw(){
  Vec ori = new Vec(-5, (width/2-mouseX)/100.0+2, 2+(height/2-mouseY)/100.0);
...
//  objs[19] = new Tri(new Vec(INF, INF, INF), new Vec(INF, INF, INF), new Vec(INF, INF, INF));
  ...
  objs[20] = new Tri(new Vec(-3, 20, -G), new Vec(-3, -20, -G), new Vec(15, 0, -G));
  objs[20].cdif = color(100, 100, 100);
  objs[20].cref = color(100, 100, 100);
  objs[20].ref = 0.0;
  objs[20].texture = tex;
  objs[21] = new Tri(new Vec(15, 0, -G), new Vec(-3, -20, -G), new Vec(15, 0, 20));
  objs[21].cdif = color(100, 100, 100);
  objs[21].cref = color(100, 100, 100);
  objs[21].ref = 0.8;
  objs[22] = new Tri(new Vec(-3, 20, -G), new Vec(15, 0, -G), new Vec(15, 0, 20));
  objs[22].cdif = color(100, 100, 100);
  objs[22].cref = color(100, 100, 100);
  objs[22].ref = 0.8;
  
  objs[23] = new Sphere(new Vec(3, 2.5, 0.5), 1.0);
  objs[23].cdif = color(100, 100, 100);
  objs[23].cref = color(100, 100, 100);
  objs[23].ref = 0.5;
  
  objs[24] = new Sphere(new Vec(4, -3.0, 0.5), 1.6);
  objs[24].cdif = color(100, 70, 70);
  objs[24].cref = color(100, 70, 70);
  objs[24].ref = 0.3;
  objs[24].n = 1.1;
  objs[24].tra = 0.8;
  ...
  for(int i=0; i<20; i++){
    ...
    objs[i].n = 1.2;
    objs[i].tra = 0.8;
    ((Tri)objs[i]).v0 = ((Tri)objs[i]).v0.add(new Vec(3, -0.4, 0));
    ((Tri)objs[i]).v1 = ((Tri)objs[i]).v1.add(new Vec(3, -0.4, 0));
    ((Tri)objs[i]).v2 = ((Tri)objs[i]).v2.add(new Vec(3, -0.4, 0));
  }
  
  Screen sc = new Screen(new Vec(2, 4, 4), new Vec(0, -8, 0), new Vec(0, 0, -8));
